...
\begin{frame}
  ...
  \item 페이지의 속성을 변경하기 위해서는, pgd -> pud -> pmd 순으로, 페이지 테이블을 찾아간 뒤
  \item 플래그를 변경하면 된다.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item ARM infocenter 에서 찾을 수 있던 어셈 코드로 테스트 했을 시, Exynos 4412 에서는 잘 동작했지만, Snapdragon 800 에서는 동작하지 않았음
  \item 이를 해결하기 위한 작업이 필요하다.
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{0xFF. TBDs}
  \framesubtitle{}
